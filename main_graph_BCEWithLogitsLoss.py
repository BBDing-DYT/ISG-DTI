...
from data_load_utils import TwoGraphDTIDataset, drug_graph_and_contact_graph_collate_fn
...
from models import CrossGraphModel
...
def load_data(config):
    ...
    train_data_set = TwoGraphDTIDataset(dataframe_train)
    val_data_set = TwoGraphDTIDataset(dataframe_val)
    test_data_set = TwoGraphDTIDataset(dataframe_test)

    data_loader_params_train = {'batch_size': config['batch_size'],
                          'shuffle': True,
                          'num_workers': config['workers'],
                          'drop_last': False,
                          'collate_fn': drug_graph_and_contact_graph_collate_fn}
...
def test(data_generator, model, dataset_name, epoch_num, is_test=False):
    ...
    for i, (atom_num, drug_graph, residue_num, contact_map, label) in enumerate(data_generator):
        logits = torch.squeeze(model(atom_num.cuda(), drug_graph.cuda(), residue_num.cuda(), contact_map.cuda()))
...
if __name__ == "__main__":
    ...
    model = CrossGraphModel(**dti_config)
    model = model.cuda()
    optimizer = torch.optim.AdamW(model.parameters(), lr=dti_config['learning_rate'], weight_decay=1e-2)
    loss_function = nn.BCEWithLogitsLoss()

    val_weights_file = "./weights/graph/" + dti_config['dataset_name'] + '_' + type(model).__name__ + "_val_weights_sigmoid.pth"
    test_weights_file = "./weights/graph/" + dti_config['dataset_name'] + '_' + type(model).__name__ + "_test_weights_sigmoid.pth"
    ...
    for epoch in range(dti_config['train_epoch']):
        model.train()
        for i, (atom_num, drug_graph, residue_num, contact_map, label) in enumerate(train_data_loader):
            score = model(atom_num.cuda(), drug_graph.cuda(), residue_num.cuda(), contact_map.cuda())
